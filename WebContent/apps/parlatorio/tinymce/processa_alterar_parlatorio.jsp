...
String varquivoCapa = multiparts.get(4).getName();
String vidConteudo = multiparts.get(5).getString();
int vidArquivoPdf = Integer.parseInt(multiparts.get(6).getString());
int vidArquivoCapa = Integer.parseInt(multiparts.get(7).getString());
...
try
{
	...
	if ( (varquivoPdf.equals("")) && (varquivoCapa.equals("")) )
	{
		// preciso alterar a edição para cada arquivo existente
		if (vidArquivoPdf > 0)
			vretorno = alterarArquivo(vdescricao, vusuario, vidConteudo, vcaminho, vidArea, vdata, vidArquivoPdf, vedicao, vdiretorio, "", vsenha, con, cs, alterar);
		if (vidArquivoCapa > 0)
			vretorno = alterarArquivo(vdescricao, vusuario, vidConteudo, vcaminho, vidArea, vdata, vidArquivoCapa, vedicao, vdiretorio, "", vsenha, con, cs, alterar);
	}
	if (!varquivoPdf.equals(""))
	{
		...
		multiparts.get(3).write(new File(vdiretorio + vnomeArquivo));
		cs = null;
		if (vidArquivoPdf > 0)
			vretorno = alterarArquivo(vdescricao, vusuario, vidConteudo, vcaminho, vidArea, vdata, vidArquivoPdf, vedicao, vdiretorio, vnomeArquivo, vsenha, con, cs, alterar);
		else
	...
	if (!varquivoCapa.equals(""))
	{
		...
		cs = null;
		if (vidArquivoCapa > 0)
			vretorno = alterarArquivo(vdescricao, vusuario, vidConteudo, vcaminho, vidArea, vdata, vidArquivoCapa, vedicao, vdiretorio, vnomeArquivo, vsenha, con, cs, alterar);
		else
