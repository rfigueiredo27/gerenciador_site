<%@ page language="java" import="java.sql.*,java.io.*, java.util.*, oracle.jdbc.OracleTypes, br.jus.trerj.funcoes.*, org.apache.commons.fileupload.*, br.jus.trerj.funcoes.IncluirGecoiArquivo,org.apache.commons.fileupload.*,be.telio.mediastore.ui.upload.UploadListener,be.telio.mediastore.ui.upload.MonitoredDiskFileItemFactory,org.apache.commons.fileupload.servlet.ServletFileUpload" %>
...
multiparts = upload.parseRequest(request);
String vid_arquivo = "";
String vid_conteudo = multiparts.get(0).getString();
String vsecao = multiparts.get(1).getString();
String vdata = multiparts.get(2).getString();
String vdescricao = multiparts.get(3).getString();
String varquivoImagem = multiparts.get(4).getName();
...
try
{
	...
	vretorno = incluir.incluir(vsecao + "@@" + vdescricao, vdiretorio, vnomeArquivo, vid_area, session.getAttribute("login").toString(), session.getAttribute("senha").toString(), vdata, "", 1, "", 0);
	vid_arquivo = vretorno.substring(vretorno.lastIndexOf("#")+1);
	vid_conteudo = vretorno.substring(vretorno.indexOf("#")+1);
	...
	vretorno = incluir.incluir(vid_conteudo, vsecao + "@@" + vdescricao, vdiretorio, varquivoImagem, session.getAttribute("login").toString(), session.getAttribute("senha").toString(), 2, 0, " ");
