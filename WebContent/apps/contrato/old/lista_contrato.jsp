...
{
	if (confirm("Deseja realmente excluir o contrato " + vdescricao + " ?") == true)
		$.post("/gecoi.3.0/apps/contrato/processa_exclusao.jsp", {idConteudo : vidConteudo}, function(){top.listar();});
}

function excluirAditivo(vidArquivo, vdescricao)
{
	if (confirm("Deseja realmente excluir o aditivo/termo ?") == true)
		$.post("/gecoi.3.0/apps/contrato/processa_exclusao_aditivo.jsp", {idArquivo : vidArquivo}, function(){top.listar()});
}
*/
</script>
<%
	...
	<c:when test="${!empty items}">
		...
		<c:forEach var="registro" items="${items}" >
			<tr>
				...
				<td><a href="#" onclick="carregaPag('/gecoi.3.0/apps/contrato/alterar_anexo.jsp?idLicitacao=${registro.idArquivo }&nPregao=${registro.numPregao }&nProcesso=${registro.numProcesso }','divbusca');"  title="Manutenção dos Contratos"><img id="aditivo${registro.idConteudo }" name="aditivo${registro.idConteudo }" src="/gecoi.3.0/img/texto.jpg" onclick="" width="30" height="30" /></a></td>
			</tr>					
		</c:forEach>
		</table>
	</form>
