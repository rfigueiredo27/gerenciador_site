...
/*String vordens = request.getParameter("ordens") =="" ? "0" : request.getParameter("ordens");
String[] aOrdens = vordens.split(",");
...
<input type="hidden" name="app" id="app" value="/gecoi.3.0/apps/contrato/lista_aditivo.jsp?idLicitacao=<%=vidLicitacao %>&descricao=<%=vdescricao %>&idConteudo=<%=vidConteudo %>&nProcesso=<%=vnumProcesso %>&nPregao=<%=vnumPregao %>&nContrato=<%=vcontrato %>&dataPublicacao=<%=vdataPublicacao %>" />
