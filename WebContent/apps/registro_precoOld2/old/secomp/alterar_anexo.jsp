...
{
	//parent.tb_remove();
	//carregaPag("/gecoi.3.0/apps/registro_preco/secomp/alterar_anexo.jsp?idLicitacao=" + document.fanexo.idLicitacao.value + "&nProcesso=" + document.fanexo.nProcesso.value + "&nPregao=" + document.fanexo.nPregao.value, "divbusca");
}
...
function excluirAnexo(vidConteudo, plicitacao, pnProcesso, pnPregao)
{
	if (confirm("Deseja realmente excluir a ata ?") == true)
		$.post("/gecoi.3.0/apps/registro_preco/secomp/processa_exclusao.jsp", {idConteudo : vidConteudo}, 
				function(){carregaPag("/gecoi.3.0/apps/registro_preco/secomp/alterar_anexo.jsp?idLicitacao=" + plicitacao + "&nPregao=" + pnPregao + "&nProcesso=" + pnProcesso,"divbusca");});
}
</script>
<%
...
<c:set var="items" value="${lista.getListaReferencia(idLicitacao, sessionScope['login'], sessionScope['senha'])}" />
		<form name="fanexo" action="/gecoi.3.0/apps/registro_preco/secomp/alterar_anexo.jsp?idLicitacao=${idLicitacao }&nProcesso=${numProcesso }&nPregao=${numPregao }" method="post" enctype="multipart/form-data">
			...
			<c:forEach var="anexo" items="${items}" >
     			...
     			<tr>
        			<td height="40" align="right" valign="middle">        			
        				<!-- <a href="/gecoi.3.0/apps/registro_preco/secomp/incluir_anexo.jsp?idConteudo=${idConteudo}&nProcesso=${numProcesso}&idArquivo=${idArquivo}&nPregao=${numPregao}">(+) Incluir Ata</a> -->
        				<a href="#" onclick="carregaPag('/gecoi.3.0/apps/registro_preco/secomp/incluir_anexo.jsp?nProcesso=${numProcesso}&idLicitacao=${idLicitacao}&nPregao=${numPregao}','divbusca');">(+) Incluir Ata</a>
        			</td>
      			</tr>
		</table>
	</form>
