...
function criticaAnexo(pLicitacao, pnPregao, pnProcesso, pedital)
{
	if (critica_altera_registro(document.fdescAnexo))
	{
		$.post("/gecoi.3.0/apps/registro_preco/secomp/processa_alteracao_dados_ata.jsp", {idConteudo: document.fdescAnexo.idconteudo.value, 
				...
				function(){
					carregaPag("/gecoi.3.0/apps/registro_preco/secomp/lista_registro.jsp?idLicitacao=" + pLicitacao + "&nPregao=" + pnPregao + "&nProcesso=" + pnProcesso + "&edital=" + pedital,"divbusca");
				});
	}
}
...
<form name="fdescAnexo" action="/gecoi.3.0/apps/registro_preco/secomp/lista_registro.jsp?idLicitacao=<%=vidLicitacao %>&nPregao=<%=vnPregao %>&nProcesso=<%=vnProcesso %>" method="post"> 
...
<input type="hidden" name="idarea" id="idarea" value="<%=vidArea%>"/>
      ...
      <div id="numero_ata">
    		<fieldset>
                ...
                <input title="Número da ata" alt="Número da ata" type="text" name="numAta" id="numAta" size="3" maxlength="3" value="<%=vnumAta %>" onblur="poeZero();" /> / <input title="Ano da ata" alt="Ano da ata" type="text" name="anoAta" id="anoAta" size="4" maxlength="4" readonly="readonly" value="<%=vano %>" />
            </fieldset>
	    </div>
      <div id="data_anexo">
    		<fieldset>
      ...
      <div id="botao">
           <input type="button" name="cancelar" value="Cancelar" onclick="carregaPag('/gecoi.3.0/apps/registro_preco/secomp/lista_registro.jsp?idLicitacao=<%=vidLicitacao%>&nPregao=<%=vnPregao%>&nProcesso=<%=vnProcesso%>&edital=<%=vedital %>','divbusca');" />
