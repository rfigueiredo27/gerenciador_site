...
public class AlteraCurriculoImg extends HttpServlet {
			...
			else
			{
				...
				catch (Exception e) 
				{
					...
					try {
						...
						// se o idArquivo estiver vazio significa que o curriculo tinha texto mas não tinha foto, por isso eu tenho que incluir com ordem = 1
						if (vidArquivo.equals(""))
						{
							IncluirGecoiArquivo incluir = new IncluirGecoiArquivo();
							vretorno = incluir.incluir(vidConteudo, vdescricao, diretorio, arquivoImgCroped, request.getSession().getAttribute("login").toString(), request.getSession().getAttribute("senha").toString(), 1, 1, "ativo");
						}
						else
						{
