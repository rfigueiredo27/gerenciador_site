...
public class GravaNovoArquivo2 extends HttpServlet {


	private static final long serialVersionUID = 1L;

	public GravaNovoArquivo2() {}
...
//		String dataPublicacao = formatarDate.format(data);
		...
		if (isMultipart) {
			...
			vobservacao = URLDecoder.decode(multiparts.get(2).getString(), "UTF-8");
			vdescricao = URLDecoder.decode(multiparts.get(3).getString(), "UTF-8");
			...
			try {
				...
				try {
					...
					for(int i=1; i<= totalAnexo; i++){
						...
						//Formatar a data caso utilize o tipo "date" do HTML 5
//						String[] data = (multiparts.get(++ultimo)).getString().split("-");
//						String ano = data[0];
//						String mes = data[1];
//						String dia = data[2];
						
						String dataPublicacao2 = ((FileItem)multiparts.get(++ultimo)).getString();
						...
						vretorno = incluir.incluir(vdescricao, diretorio, arquivoObjeto, vidArea, request.getSession().getAttribute("login").toString(), request.getSession().getAttribute("senha").toString(), dataPublicacao2, "", 0, vobservacao, 0);
						vidArquivo = vretorno.substring(vretorno.lastIndexOf("#") + 1);
