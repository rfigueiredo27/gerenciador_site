...
import br.jus.trerj.modelo.AcessoRapido;
import br.jus.trerj.modelo.Parametros;

public class ListaAcessoRapido {
	//int vidArea = parametros.getVidAreaContrato();


	public List<AcessoRapido> getListaAcessoRapido(String vusuario, String vsenha) throws ClassNotFoundException, SQLException{
		...
		List<AcessoRapido> documentos = new ArrayList<AcessoRapido>();
		int idArea = 1807; //Área correspondente a Revista Jurisprudencia
		...
		while (rs.next())
		{
			AcessoRapido gecoi = new AcessoRapido();
			...
			String[] observacao = rs.getString("obs").split("@");
			
			gecoi.setLink(observacao[0]);
			gecoi.setHint(observacao[1]);
			gecoi.setTarget(observacao[2]);
