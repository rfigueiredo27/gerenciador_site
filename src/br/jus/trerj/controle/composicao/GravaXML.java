...
public class GravaXML extends HttpServlet implements Servlet{
			...
			try {
	     		...
	     		try 
	    		{
	    			...
	    			stream.toXML(listaComponentes, arquivoXML);
	     			...
	     			String vsql = "{call gecoi.g_processar_incluir_arquivo(?, ?, null, ?, " + 
	    					"?, Sysdate, Sysdate+365, " +
	    					"?, ?)}";
	    			...
	    			cs.setString(2,"Composição da Corte"); //descricao
	    			cs.setString(3,request.getSession().getAttribute("login").toString()); //usuario
	    			
	    			// variáveis da inclusão de conteudo_area
	    			cs.setInt(4, idArea); //vid_area
	    				    			
	    			// variáveis da inclusão de arquivo
	    			FileInputStream fis = new FileInputStream(arquivo);
	    			cs.setBinaryStream(5, fis, (int)arquivo.length());
	    			//cs.setBinaryStream(5,arquivo.getInputStream(), (int) arquivo.getSize()); //arquivo
	    			//cs.setString(6,vnomeArquivo.substring(vnomeArquivo.lastIndexOf(".")));//nome
